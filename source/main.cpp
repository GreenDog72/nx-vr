...
static void sceneInit()
{
    ...
    auto projMtx = glm::perspective(40.0f*TAU/360.0f, 8.0f/9.0f, 0.01f, 1000.0f);
...
static int s_scrWidth = 1280;
static int s_scrHeight = 720;
...
static void sceneUpdate(u32 kHeld)
{
    ...
    mdlvMtx = glm::translate(mdlvMtx, glm::vec3{eye_off, 0.0f, 0.0f});
...
static void configureResolution(NWindow* win, bool half_horiz, bool half_vert)
{
    int width, height;
...
static void scenePreRender()
{
    glClearColor(0x68/255.0f, 0xB0/255.0f, 0xD8/255.0f, 1.0f);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

static void sceneRender(float eye_off)
{
    //glClearColor(0x68/255.0f, 0xB0/255.0f, 0xD8/255.0f, 1.0f);
    //glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
int main(int argc, char* argv[])
{
    ...
    while (appletMainLoop())
    {
        ...
        bool shouldHalveHorizResolution = !!(kHeld & KEY_A);
        bool shouldHalveVertResolution = !!(kHeld & KEY_B);
        ...
        configureResolution(win, shouldHalveHorizResolution, shouldHalveVertResolution);
        ...
        glViewport(0, 1080 - s_scrHeight, s_scrWidth / 2, s_scrHeight);
        sceneRender(0.08f);
        glViewport(s_scrWidth / 2, 1080 - s_scrHeight, s_scrWidth / 2, s_scrHeight);
        sceneRender(-0.08f);
